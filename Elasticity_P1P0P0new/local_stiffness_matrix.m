...
%       ps=[1 0 0 0 0 1;0 1 0 1 0 0];
     ...
     for i=1:3%A1take A2take
         for j=1:3
             A1(5+2*i,5+2*j)=N(1,i)*N(1,j)/T+N(2,i)*N(2,j)/(2*T);
             A1(6+2*i,5+2*j)=N(2,j)*N(1,i)/(2*T);
             A1(5+2*i,6+2*j)=N(2,i)*N(1,j)/(2*T);
             A1(6+2*i,6+2*j)=N(1,i)*N(1,j)/(2*T)+N(2,i)*N(2,j)/T;
             A2(5+2*i,5+2*j)=N(1,i)*N(1,j)/T;
             A2(6+2*i,5+2*j)=N(1,j)*N(2,i)/T;
             A2(5+2*i,6+2*j)=N(1,i)*N(2,j)/T;
             A2(6+2*i,6+2*j)=N(2,i)*N(2,j)/T;
         end
     end
     %%内部内部稳定项
     TE=eye(3);
     ...
     for k=1:3
        ...
        for i=1:3
            ...
            fun2=@(x,y)[TEMP(1)+TEMP(2)*x+TEMP(3)*y;0];%当(x,y)=(x_i,y_i)时方程为1，其他两点为0，
            b=QB2(fun2,pk);%QB2带入的是pk，当pk端点有(x_i,y_i)时方程在这点值为1，不是则为0，所以在求QB时，方程与P的端点顺序无关
            A3(i,2*k+5:2*k+6)=-b'*K;
            A3(2*k+5:2*k+6,i)=-K*b;
            fun1=@(x,y)[0;TEMP(1)+TEMP(2)*x+TEMP(3)*y];
            b=QB2(fun1,pk);
            A3(i+3,2*k+5:2*k+6)=-b'*K;
            A3(2*k+5:2*k+6,i+3)=-K*b;
        end
        %%%边界边界
        A3(2*k+5:2*k+6,2*k+5:2*k+6)=K;
